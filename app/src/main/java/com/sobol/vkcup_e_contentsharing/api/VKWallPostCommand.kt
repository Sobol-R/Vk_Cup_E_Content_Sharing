package com.sobol.vkcup_e_contentsharing.api

import android.net.Uri
import com.sobol.vkcup_e_contentsharing.models.VKFileUploadInfo
import com.sobol.vkcup_e_contentsharing.models.VKSaveInfo
import com.sobol.vkcup_e_contentsharing.models.VKServerUploadInfo
...
class VKWallPostCommand(
    ...
    override fun onExecute(manager: VKApiManager): Int {
        ...
        if (ownerId != 0) {
            println("OWNER ID NOT NULL")
            callBuilder.args("owner_id", ownerId)
        }

        if (photos.isNotEmpty()) {
            println("PHOTOS IS NOT EMPTY")
    ...
    private fun getServerUploadInfo(manager: VKApiManager): VKServerUploadInfo {
        val uploadInfoCall = VKMethodCall.Builder()
            ...
            .version(manager.config.version)
            .build()
        return manager.execute(uploadInfoCall,
    ...
    private fun uploadPhoto(uri: Uri, serverUploadInfo: VKServerUploadInfo, manager: VKApiManager): String {
        val fileUploadCall = VKHttpPostCall.Builder()
            ...
            .build()
        val fileUploadInfo = manager.execute(fileUploadCall, null,
        ...
        val saveInfo = manager.execute(saveCall,
            SaveInfoParser()
        )
